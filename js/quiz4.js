...
const completed = localStorage.getItem("pinocchio_level4Completed") === "true";
const puzzleScoreRaw = localStorage.getItem("pinocchio_level4PuzzleScore");
...
if (!DEV_MODE && !completed && puzzleScoreRaw === null) {
  window.location.href = "../html/level_4.html";
}
...
function finishEvaluation() {
  ...
  localStorage.setItem("pinocchio_level4Completed", "true");
  localStorage.setItem("pinocchio_level4Score", levelTotal.toString());
  renderHumanityBadge("humanityBadge");
...
function showResultsOnly() {
  const storedScore = parseInt(localStorage.getItem("pinocchio_level4Score") || "0", 10);
  resultsContent.innerHTML = buildResultsHTML({
    puzzleScore: null,
    mcCorrect: null,
    evalScore: null,
...
function buildResultsHTML({ puzzleScore, mcCorrect, evalScore, levelTotal, newTotal, alreadyDone }) {
  ...
  return `
    <p class="quiz-eyebrow">Level 4 complete</p>
