/* =========================================================
   quiz.js
   Step-based evaluation for Level 3 ("The Audiobook").

   - 4 multiple-choice questions about the AUDIOBOOK as a medium
...
const completed = localStorage.getItem("pinocchio_level3Completed") === "true";
const puzzleScoreRaw = localStorage.getItem("pinocchio_level3PuzzleScore");
...
if (!DEV_MODE && !completed && puzzleScoreRaw === null) {
  window.location.href = "../html/level_3.html";
}
...
function finishEvaluation() {
  ...
  localStorage.setItem("pinocchio_level3Completed", "true");
  localStorage.setItem("pinocchio_level3Score", levelTotal.toString());
  renderHumanityBadge("humanityBadge");
...
function showResultsOnly() {
  const storedScore = parseInt(localStorage.getItem("pinocchio_level3Score") || "0", 10);
  resultsContent.innerHTML = buildResultsHTML({
    puzzleScore: null,
    mcCorrect: null,
    evalScore: null,
